...
with graph2:
    second_question = get_data('data/pacientes_ano.csv')
    fig2 = px.bar(second_question.reset_index(), x='ANO', y='PACIENTES', title='Pacientes com CID L20 atendidos por ano')
...
with indicator2:
    third_question = get_data('data/tipo_atendimento.csv')
    fig3 = px.bar(third_question.reset_index(),x='ANO',y='PACIENTES',color='TIPO_ATENDIMENTO',title='Quantos pacientes atendidos em serviços de emergência? E quantos são ambulatoriais?')
...
with graph3:
    ...
    internacoes = get_data('data/internacao_custos.csv')
    internacoes = px.bar(internacoes.reset_index(), x = 'FAIXA_ETARIA',y='NUM_PROCEDIMENTOS',title='Quantidade de internações por faixa etária')
...
with indicator3:
    ...
    pergunta4 = get_data('data/age_groups.csv')
    st.write('-')
...
with graph4:
    tempo_internacao = get_data('data/tempo_internacao.csv')
    tempo_internacao = px.bar(tempo_internacao.reset_index(), x='ANO_CMPT', y='MEDIA_DIAS_INTERNACAO', title='Qual o tempo médio de dias de internação de pacientes com CID L20?',
                  labels={'ANO_CMPT':'ANO'})
...
with third_question:
    st.subheader('Qual a relação dos custos indiretos (licença médica, aposentadora precoce, DALY, etc) dos pacientes com dermatite atópica moderada a grave e seus cuidadores, comparando a outras doenças dermatológicas?')

graph6, blank6, indicator6 = st.columns(3)


with graph6:
    beneficios_doenca = get_data('data/beneficios_doenca.csv')
    beneficios_doenca = px.bar(beneficios_doenca.reset_index(),x='ANO',y='NUM_BENEFICIOS',color='CID_SHORT',title= 'Quantos benefícios solicitados e concedidos comparativamente entre as doenças?',barmode='group')
...
with indicator6:
    beneficios_idade = get_data('data/beneficios_idade.csv')
    graph_beneficios_age = px.bar(beneficios_idade.reset_index(),x='CID_SHORT',y='NUM_BENEFICIOS',color='FAIXA_ETARIA',title='Qual a faixa etária que mais solicita benefícios em cada uma das doenças?',barmode="group",labels={'CID_SHORT': "CID"
                 })
...
with graph7:
    beneficios_tipo = get_data('data/beneficios_tipos.csv')
    graph_beneficios_tipo = px.bar(beneficios_tipo.reset_index(),x='CID_SHORT',y='NUM_BENEFICIOS',color='Espécie',barmode='group',title='Quais tipos de benefícios mais solicitados e mais concedidos?')
...
with indicator7:
    df_daly = get_data('data/df_daly.csv')
